export default class AppController {
    ...
    async initialize() {
        ...
        this.dragAndDropManager.initialize(this.onFileChange.bind(this));
    ...
    async onProgress({ alreadyProcessed, filename }) {
        ...
        const percentageProcessed = Math.ceil((alreadyProcessed / file.size) * 100);

        this.updateProgress(file, percentageProcessed);

        if (percentageProcessed < 98) return;

        return this.updateCurrentFiles();
    }

    // used to calculate the uploading percentage of total of items
    ...
    async updateCurrentFiles() {
        const files = await this.connectionManager.getCurrentFiles();
